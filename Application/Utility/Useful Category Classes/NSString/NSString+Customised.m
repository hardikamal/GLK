...
+ (NSString *)humanReadableDateFrom:(NSDate *)date {
	BOOL isFutureDate = [[NSDate date] isEarlierThanDate:date];
	if (isFutureDate) {
		return [date humanIntervalSinceNow];
	}
	else {
		return [date humanIntervalAgo];
	}
}

- (NSString *)humanReadableDate {
	return [[NSString humanReadableDateFrom:[NSString formatStringToDate:self]] lowercaseString];
}
