...
{
    NSArray* sorted = fn();
    return [NSArray arrayWithObjects:[sorted first], [sorted last], nil];
}
...
{
    __block NSMutableArray* notDropped = [[NSMutableArray alloc] init];
...
{
    return [self filter:fn stopOnFind:NO];
}
...
{
    return [self reduce:acc fn:fn];
}

-(NSDictionary*)groupBy:(id(^)(id obj))fn
{    
    __block NSMutableDictionary* groups = [[NSMutableDictionary alloc] init];
...
{
    __block NSMutableArray* map = [[NSMutableArray alloc] initWithCapacity:[self count]];
...
-(NSArray*)partition:(BOOL(^)(id obj))fn
{
    ...
    NSArray* allVals = [NSArray arrayWithObjects:falseVals, trueVals, nil];
...
{
    return [self sortedArrayUsingComparator:fn];
}
