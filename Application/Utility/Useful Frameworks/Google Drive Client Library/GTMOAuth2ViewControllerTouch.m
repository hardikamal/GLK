...
- (void)viewDidLoad
{
  ...
  self.navigationItem.rightBarButtonItem = rightBarButtonItem_;
    ...
    UINavigationItem *navigItem = [[UINavigationItem alloc] initWithTitle:@"Sign in - Google Accounts"];
...
- (BOOL)removePasswordForService:(NSString *)service account:(NSString *)account error:(NSError **)error
{
  ...
  if (status != noErr && error != NULL)
  {
    *error = [NSError errorWithDomain:kGTMOAuth2KeychainErrorDomain  code:status  userInfo:nil];
  }
  return status == noErr;
}
// iPhone
- (BOOL)setPassword:(NSString *)password  forService:(NSString *)service accessibility:(CFTypeRef)accessibility
