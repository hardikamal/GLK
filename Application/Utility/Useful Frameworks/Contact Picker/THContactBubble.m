...
#define kBubbleBorderColor                [UIColor lightGrayColor]
#define kBubbleColor                      [UIColor whiteColor]
#define kBubbleColorSelected              [UIColor whiteColor]
...
- (id)initWithName:(NSString *)name color:(THBubbleColor *)color selectedColor:(THBubbleColor *)selectedColor {
    ...
    if (self) {
        ...
        if (color == nil) {
            color = [[THBubbleColor alloc] initWithGradientTop:kBubbleColor gradientBottom:kBubbleColor border:kBubbleBorderColor];
        }
        
        if (selectedColor == nil) {
            selectedColor = [[THBubbleColor alloc] initWithGradientTop:kBubbleColorSelected gradientBottom:kBubbleColorSelected border:kBubbleBorderColor];
        }
...
- (void)unSelect {
    ...
    [self removeContactButton];
}

- (void)addContactButton {
#define B_H 40
#define B_W 70
    if ([self viewWithTag:1024] == nil) {
        UIButton *cross = [UIButton buttonWithType:UIButtonTypeCustom];
        [cross addTarget:self action:@selector(deleteClicked) forControlEvents:UIControlEventTouchUpInside];
        UIImageView * crossIcon = [cross viewWithTag:1025];
        if (crossIcon==nil) {
            crossIcon = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"cross"]];
        }
        [cross addSubview:crossIcon];
        [self addSubview:cross];
        [cross setFrame:CGRectMake(self.frame.size.width - 50, -21, B_W, B_H)];
        [crossIcon setFrame:CGRectMake(((B_W-16)/2),(B_H-16)/2,16,16)];
        [cross setTag:1024];
    }
}
