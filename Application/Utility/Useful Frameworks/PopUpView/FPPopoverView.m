...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _titleLabel.font = [UIFont fontWithName:Embrima size:16];
...
- (void)drawRect:(CGRect)rect
{
    ...
    //external border
    CGPathRef externalBorderPath = [self newContentPathWithBorderWidth:0.0 arrowDirection:_arrowDirection];
    CGContextBeginPath(ctx);
...
-(void)setupViews
{
    ...
    else if(_arrowDirection == FPPopoverNoArrow)
    {
        ...
        _titleLabel.frame = CGRectMake(10, 10, self.bounds.size.width-20, 20);
		if (self.title==nil || self.title.length==0) {
			contentRect.origin = CGPointMake(0, 5);
