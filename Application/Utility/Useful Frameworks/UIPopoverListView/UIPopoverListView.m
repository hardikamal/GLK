...
- (void)defalutInit
{
    ...
    self.listView.alwaysBounceVertical = NO;
    ...
    if ([_titleView.text length]!=0)
    {
        ...
        _titleView.backgroundColor = [UIColor colorWithRed:13/255.0 green:198/255.0 blue:170/255.0 alpha:1.0f];
    ...
    {
        CGFloat xWidth = self.bounds.size.width;
        CGRect tableFrame = CGRectMake(0, 0.0f, xWidth, self.bounds.size.height);
        _listView = [[UITableView alloc] initWithFrame:tableFrame style:UITableViewStylePlain];
        _listView.dataSource = self;
        _listView.delegate = self;
        [self addSubview:_listView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   static NSString *identifier = @"cell";
    ...
    cell.textLabel.textColor=[UIColor darkGrayColor];
     cell.textLabel.font=[UIFont fontWithName:@"Halvetica-Light" size:15];
    return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString * noticationName =self.NotificationName;
    NSMutableDictionary *bookListing = [[NSMutableDictionary alloc] init];
    [bookListing setObject:[_listArray objectAtIndex:[indexPath row]] forKey:@"object"];
    if ([_titleView.text length]!=0)
    {
         [bookListing setObject:_titleView.text forKey:@"title"];
    }else
        [bookListing setObject:@"" forKey:@"title"];
    
    [bookListing setObject:[NSNumber numberWithInt:self.tag] forKey:@"tag"];
