...
- (instancetype)initWithDelegate:(id<THPinViewDelegate>)delegate
{
    ...
    if (self)
    {
        ...
        UIUnderlinedButton *fromLabel = [[UIUnderlinedButton alloc] init];
        [fromLabel setTitle:NSLocalizedString(@"forgotPassword", nil) forState:UIControlStateNormal];
        [fromLabel.titleLabel setFont:[UIFont fontWithName:Embrima size:16]];
        fromLabel.titleLabel.font=[UIFont fontWithName:Ebrima_Bold size:14];
        [fromLabel addTarget:self action:@selector(showLeft) forControlEvents:UIControlEventTouchUpInside];

        fromLabel.backgroundColor = [UIColor clearColor];
        fromLabel.tintColor = [UIColor whiteColor];
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            ...
            if (isFourInchScreen)
            {
                ...
                _paddingBetweenInputCirclesAndNumPad = 71.5f;
                _paddingBetweenNumPadAndBottomButton = 19.0f;
                [fromLabel setFrame:CGRectMake(10, 60, 250, 20)];
            } else
            {
                _paddingBetweenPromptLabelAndInputCircles = 20.5f;
                _paddingBetweenInputCirclesAndNumPad = 40.0f;
                _paddingBetweenNumPadAndBottomButton = -30.5f;
                [fromLabel setFrame:CGRectMake(10, 40, 250, 20)];
...
- (void)updateBottomButton
{
    if ([self.input length] == 0)
    {
        ...
        [self.bottomButton setTitle:NSLocalizedString(@"cancel_button_title", nil)forState:UIControlStateNormal];
    ...
    } else
    {
        ...
        [self.bottomButton setTitle:NSLocalizedString(@"delete_button_title", nil) forState:UIControlStateNormal];
